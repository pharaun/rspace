        ...
        loop {
            ...
            match (func7, func3, opcode) {
                ...
                (        _, 0b000, opcode::OP_IMM) => {
                    // ADDI
                    // TODO: add as signed?
                    self.reg[rd] = self.reg[rs1] + sign_extend(inst, Iimm);
                },
                ...
                (        _, 0b010, opcode::OP_IMM) => {
                    // SLTI
                    self.reg[rd] = if (self.reg[rs1] as i32) < (sign_extend(inst, Iimm) as i32) { 0x1 } else { 0x0 }
                },
                (        _, 0b011, opcode::OP_IMM) => {
                    // SLTIU
                    self.reg[rd] = if self.reg[rs1] < sign_extend(inst, Iimm) { 0x1 } else { 0x0 }
                },
                ...
                (        _, 0b000, opcode::JALR) => {
                    ...
                    //self.pc = (self.reg[rs1] + Iimm) as usize;
                ...
                (        _, 0b000, opcode::LOAD) => {
                    ...
                    let byte = self.mem[(self.reg[rs1] + sign_extend(inst, Iimm)) as usize];
                ...
                (        _, 0b001, opcode::LOAD) => {
                    ...
                    // TODO: abstract this to memory?
                    let bytes: [u8; 2] = [
                        self.mem[(self.reg[rs1] + sign_extend(inst, Iimm)) as usize],
                        self.mem[(self.reg[rs1] + sign_extend(inst, Iimm) + 1) as usize]
                    ];
                ...
                (        _, 0b010, opcode::LOAD) => {
                    ...
                    let bytes: [u8; 4] = [
                        self.mem[(self.reg[rs1] + sign_extend(inst, Iimm)) as usize],
                        self.mem[(self.reg[rs1] + sign_extend(inst, Iimm) + 1) as usize],
                        self.mem[(self.reg[rs1] + sign_extend(inst, Iimm) + 2) as usize],
                        self.mem[(self.reg[rs1] + sign_extend(inst, Iimm) + 3) as usize]
                    ];
                ...
                (        _, 0b000, opcode::STORE) => {
                    // SB
                    // TODO: abstract this to memory?
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm)) as usize] = (self.reg[rs2] & 0x00_00_00_FF) as u8;
                },
                (        _, 0b001, opcode::STORE) => {
                    // SH
                    // TODO: abstract this to memory?
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm)) as usize]     = (self.reg[rs2] & 0x00_00_00_FF) as u8;
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm) + 1) as usize] = (self.reg[rs2] & 0x00_00_FF_00) as u8;
                },
                (        _, 0b010, opcode::STORE) => {
                    ...
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm)) as usize]     = (self.reg[rs2] & 0x00_00_00_FF) as u8;
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm) + 1) as usize] = (self.reg[rs2] & 0x00_00_FF_00) as u8;
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm) + 2) as usize] = (self.reg[rs2] & 0x00_FF_00_00) as u8;
                    self.mem[(self.reg[rs1] + sign_extend(inst, Simm) + 3) as usize] = (self.reg[rs2] & 0xFF_00_00_00) as u8;
                },
                ...
                (        _, 0b000, opcode::BRANCH) => {
                    ...
                    //    self.pc += (sign_extend(inst, SBimm) as usize);
                    //}
                },
                ...
                (        _,     _, opcode::AUIPC) => {
                    // AUIPC
                    self.reg[rd] = Uimm + (self.pc as u32);
                },
                (        _,     _, opcode::JAL) => {
                    // JAL
                    // TODO: unclear if we need to execute the jumped to instruction or +4?
                    //self.pc += sign_extend(inst, UJimm) as usize;
                    //self.reg[rd] = (self.pc + 4) as u32;
                },
